...
@implementation TutorialStepLocal

- (id)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        self.illustration = [[UIImageView alloc]initWithFrame:CGRectMake(0, (self.frame.size.height - 480)/2 + 20, 320, 480)];
        self.illustration.image = [UIImage imageNamed:@"Tutorial_Step1"];
        self.illustration.alpha = 0;
        [self addSubview:_illustration];
        
        self.text = [[UILabel alloc]initWithFrame:CGRectMake(30,  (self.frame.size.height - 480)/2 + 210 + 40, 260, 180)];
        self.text.backgroundColor = [UIColor clearColor];
        self.text.textColor = [UIColor whiteColor];
        self.text.numberOfLines = 3;
        self.text.textAlignment= NSTextAlignmentCenter;
        self.text.font = [UIFont fontWithName:@"Lato-Bold" size:19];
        self.text.text = NSLocalizedString(@"These are activities where you and everyone around is invited.", nil);
        self.text.alpha = 0;
        [self addSubview:_text];
        
        self.information = [[TTTAttributedLabel alloc]initWithFrame:CGRectMake(0, (self.frame.size.height - 480)/2 + 380 + 60, 320, 20)];
